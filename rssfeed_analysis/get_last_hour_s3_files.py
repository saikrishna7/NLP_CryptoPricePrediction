...
class GetTestData:
    ...
    def get_time(self, duration=0):
        dt = datetime.utcnow() - timedelta(hours=duration)
        if duration>1:
    ...
    def get_list_of_files_from(self, date, duration=0):
        ...
        order_files=[]
        if duration>1:
            files_path = "rssfeed_firehose/rssfeed/raw-data/" + date + "/"
            response = GetTestData.s3_client.list_objects(
                Bucket=GetTestData.bucket_name,
                Prefix=files_path
            )
            # print("last day files: ", response)
            files = {}
            ...
            # print("last hour files: ", response)
            files = {}
            # print(response)
    ...
    def main(self, duration=24):
        ...
        print("type of duration:", type(duration))
